...
func (h *Handler) create(w http.ResponseWriter, r *http.Request) {
	...
	if in.TTLMinutes != nil && *in.TTLMinutes > 0 {
		v := time.Now().Add(time.Duration(*in.TTLMinutes) * time.Minute)
		exp = &v
	}

	c, cancel := reqCtx(r); defer cancel()
	n, err := h.repo.Create(c, in.Title, in.Content, exp)
...
// GET /api/v1/notes?q=...&limit=&skip=
// Поиск через $text, если q не пустой
...
func (h *Handler) patch(w http.ResponseWriter, r *http.Request) {
	...
	c, cancel := reqCtx(r); defer cancel()
	n, err := h.repo.Update(c, id, in.Title, in.Content, in.TTLMinutes)
	if errors.Is(err, ErrNotFound) { writeJSON(w, 404, map[string]string{"error":"not_found"}); return }
	if err != nil { writeJSON(w, 500, map[string]string{"error": err.Error()}); return }
	writeJSON(w, 200, n)
}
